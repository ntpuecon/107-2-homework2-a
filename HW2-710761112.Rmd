...
summary(TSLSHW13)
#加了工具變數以後和1.1那題相比，educ前的係數由-0.09到-0.17，會有正向偏誤，實際的educ的效應值應該越大
...
#電視的關聯是負效果的原因:1.有可能有輻射，對爸爸媽媽的生育能力有負面的影響
                      #2.因為看電視以後，對社會失望，對未來的世界擔憂，所以不想要生小孩
...
不一定有一致性，因為後面兩項$E(y_{-x,-w,i}|\tilde{x},w_i)-E(y_{-x,-w,i}|\tilde{x},w_i)$有可能不等於零表示說$\tilde{x}_i$和$w_i$和$E(y_{-x,-w,i}|\tilde{x},w_i)-\beta^*E(\epsilon|\tilde{x},w_i)$不獨立，立足點不同，會有選擇性偏誤。
